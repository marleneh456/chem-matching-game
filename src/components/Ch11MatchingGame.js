...
// Replace this with your original full list of default flashcards
const defaultItems = [
  {
    id: 1,
...
const MatchingGame = () => {
  ...
  const removeMatch = (termId) => {
    const removedTerm = matches[termId];
    setMatches((prev) => {
      const updated = { ...prev };
  ...
  return (
    <div className="game-container">
      ...
      <div className="add-flashcard">
        ...
        <div className="upload-box" onDrop={(e) => handleImageDrop(e, setNewTermImage)} onDragOver={(e) => e.preventDefault()}>
          Drag & Drop Term Image Here
          <input type="file" onChange={(e) => handleImageUpload(e, setNewTermImage)} />
        </div>
        <div className="upload-box" onDrop={(e) => handleImageDrop(e, setNewDefinitionImage)} onDragOver={(e) => e.preventDefault()}>
          Drag & Drop Definition Image Here
          <input type="file" onChange={(e) => handleImageUpload(e, setNewDefinitionImage)} />
        </div>
      ...
      <div className="delete-flashcard">
        ...
        {initialItems.map((item) => (
          <div key={item.id}>
            <button onClick={() => deleteFlashcard(item.id)}>
              Delete "{item.term}"
            </button>
          </div>
        ))}
